...
const RemoveWandoosByEmail = ({
  ...
  useEffect(() => {
    ...
    setTotalBalance(user?.wallet?.balance);
  }, []);

  const removeWandoOs = async () => {
  ...
  return (
    ...
    >
      ...
      <Text style={{ ...commonStyles.headerTitle, alignSelf: 'flex-start', marginBottom: 20 }}>Remove wandoOs</Text>

      <>
        {showTravelerInfo()}

        <View style={{ ...styles.amountContainer, ...styles.amountRow, paddingRight: 0, marginTop: 30 }}>
        ...
        <EmptyGap />
        <BlueButton
          title='Remove WandoOs'
          width={width * 0.75}
          style={{ marginTop: 30 }}
          onPressListener={removeWandoOs}
        />
...
export default RemoveWandoosByEmail;